...
class IndividualFactory:
    ...
    def getIndividual(self, individualType, father=None, mother=None):
        ind = individualType(father, mother)
        return ind

class EvolutionnaryProcess:
    def __init__(self, problem, individualType):
        ...
        for i in range(Parameters.individualsNb):
            self.population.append(IndividualFactory.getInstance().getIndividual(individualType))
        ...
        while (self.generationNb < Parameters.generationMaxNb and self.bestFitness > Parameters.minFitness):
            ...
            for i in range(Parameters.individualsNb - 1):
                if(random.random() <= Parameters.crossoverRate):
                    ...
                    ind = IndividualFactory.getInstance().getIndividual(self.individualType, father=father, mother=mother)
                else :
                    father = self.Selection()
                    ind = IndividualFactory.getInstance().getIndividual(self.individualType, father=father)
...
if __name__ == "__main__":
    ...
    gen = EvolutionnaryProcess(TSP, TSPIndividual)
    gen.Run()
